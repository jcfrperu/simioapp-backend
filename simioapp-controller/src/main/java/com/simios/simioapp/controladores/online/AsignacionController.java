...
public class AsignacionController extends BaseController {
    ...
    private static final Logger log = Logger.getLogger(AsignacionController.class.getName());

    private static final String plantilla = "default";
    private static final String modulo = "panel-asignacion";
    ...
    public ModelAndView mostrarPanelAsignacion(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            setVistaTemplate(model, "panel-asignacion", modulo, plantilla);
            ...
            // agregar los inventariadores y un valor sin asignar (marcar por default el todos)
            ComboWeb cboInventariador = cargarComboTodos(request, "cboInventariador", itemsInventariador);
            cboInventariador.addItemSelIni(Constantes.COMBO_INVENTARIADOR_SIN_ASIGNAR_VALUE, Constantes.COMBO_INVENTARIADOR_SIN_ASIGNAR_LABEL);
            cboInventariador.setSelID(Constantes.COMBO_TODOS_VALUE);

            // agregar los inventariadores Anterior
            cargarComboTodos(request, "cboInventariadorAnterior", itemsInventariador);

            // combo de inventariador a asignar
    ...
    private void handleNoExistenInventariosActivos(Map<String, Object> model, int entidadID) throws Exception {
        ...
        if (CollectionUtils.isEmpty(results)) {
            model.put("msgHidden", "No existen inventarios abiertos para asignar");
        }
    ...
    public ModelAndView buscarInventarioBien(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            String inventariadorAnterior = JS.toBlank(request.getParameter("txt_inventariador_anterior"));
            ...
            // setear siguiente vista
            setVistaPage(model, "panel-asignacion-grilla", modulo, plantilla);
            ...
            if (!JS._vacio(inventariadorAnterior)) params.put("inventariadorAnterior", inventariadorAnterior);
            ...
            // caso especial, el inventariador puede venir con valores especiales de control
            if (StringUtils.equals(inventariador, Constantes.COMBO_INVENTARIADOR_SIN_ASIGNAR_VALUE)) {
                params.put("inventariador_type", "is_null");
            }
            ...
            model.put("infoExtraHidden", createInfoResumenAsignacion(listaInventarioBien));
    ...
    private String createInfoResumenAsignacion(List<Map<String, Object>> bienes) throws Exception {
        ...
        if (CollectionUtils.isNotEmpty(bienes)) {
            ...
            List<String> listaSinAsignar = new ArrayList<>(nroBienes);

            // buscando los no asignados
            for (Map<String, Object> bien : bienes) {

                if (StringUtils.isBlank(MapUtils.getString(bien, "inventariador"))) {
                    listaSinAsignar.add(MapUtils.getString(bien, "codigoPatrimonial", StringUtils.EMPTY));
                    nroSinAsignar++;
                }
            }

            info.put("nroBienes", String.valueOf(nroBienes));
            info.put("nroSinAsignar", String.valueOf(nroSinAsignar));
            info.put("nroAsignados", String.valueOf(nroBienes - nroSinAsignar));
            info.put("listaSinAsignar", listaSinAsignar);
        }
    ...
    public ModelAndView asignar(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (StringUtils.isBlank(usuarioID) || StringUtils.equals(usuarioID, Constantes.COMBO_SELECCIONE_VALUE)) {
                throw new Exception("Parámetros incorrectos para la petición");
            }
            ...
            ArrayList<String> inventarioBienIDList = JSONUtil.fromJSONToArrayList(bienesJSON);

            List<Integer> ids = ListaUtil.parseToNumberList(inventarioBienIDList);

            inventarioBienService.asignarBienes(ids, usuarioID);
    ...
    public ModelAndView desasignar(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            inventarioBienService.desasignarBienes(ids);
