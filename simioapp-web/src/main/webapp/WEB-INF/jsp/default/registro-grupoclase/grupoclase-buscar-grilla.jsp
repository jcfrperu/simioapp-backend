...
<table class="table table-striped table-bordered table-hover" id="grupoclase-buscar-grilla">
    ...
    <tbody>
        ...
        <c:forEach items="${lista}" var="item" varStatus="status">
            ...
            <tr class="${claseFila}">
                <td class="text-center"><input type="radio" name="codigo" title="Seleccionar" value="${item.grupoID},${item.claseID}"></td>
				<td>${item.grupoCodigo} - ${item.grupoDescrip}</td>
                <td>${item.claseCodigo} - ${item.claseDescrip}</td>
                <td>${item.cuenta}</td>
