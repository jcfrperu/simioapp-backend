...
<table class="table table-striped table-bordered table-hover" id="empleado-buscar-grilla">
    ...
    <tbody>
        ...
        <c:forEach items="${lista}" var="item" varStatus="status">
            ...
            <tr class="${claseFila}">
                <td class="text-center"><input type="radio" name="codigo" title="Seleccionar" value="${item.empleadoID}"></td>
                <td title="Empleado ID: ${item.empleadoID}">${item.codigo}</td>
                <td>${item.nombres}</td>
            	<td>${item.apellidos}</td>
            	<td>${item.tipoDocIdentidad}</td>
            	<td>${item.numeroDocIdentPersonal}</td>
