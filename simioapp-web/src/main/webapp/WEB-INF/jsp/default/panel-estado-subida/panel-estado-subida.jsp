...
<div class="panel panel-info">
    <div class="panel-heading"> <strong> PANEL ESTADO DE SUBIDAS </strong> </div>
    <div class="panel-body">
                    ...
                    <div class="panel-body">
                        <div class="table-responsive" id="div-load-grilla">
                            <c:import url="/admin.htm">
                                ...
                                <c:param name="_page" value="panel-estado-subida-grilla" />
                                <c:param name="_module" value="panel-estado-subida" />
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <form id="form-inventariadores" action="estado-subida-inventario.htm" method="POST" role="form">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="control-label">Estado Subida a guardar</label>
                                <select class="form-control input-sm" title="Estado Subida a guardar" id="cbo_estado_subida_guardar" name="cbo_estado_subida_guardar">
                                    <c:forEach items="${cboEstadoSubidaGuardar.items}" var="item" >
                                        <option <c:if test="${item.id == cboEstadoSubidaGuardar.idSeleccionado}">selected</c:if> value="${item.id}">${item.label}</option>
                                    </c:forEach>
                                </select>
                                <span class="help-block"></span>
                            </div>
...
<script>
    ...
    function getRowsGrilla() {
        ...
        var table = $('#panel-estado-subida-grilla').DataTable();
    ...
    function initGrilla() {
        ...
        initDG('#panel-estado-subida-grilla', true, true, true, null, null);
        customEventSelectOnTR('#panel-estado-subida-grilla');
        eventSeleccionarTodos();
    }
    ...
    function cargarGrilla() {
        ...
        $('#div-load-grilla').load('estado-subida-inventario.htm?action=buscarInventarioBien', paramsBuscar, function(response, status, xhr) {
    ...
    function cargarInfoResumen() {
        ...
        if ( infoExtraHidden != '' ) {
            ...
            if ( estaDefinido( infoResumen ) ) {
                ...
                html = html + '<br />';
                html = html + '<span class="label label-danger">BIENES SIN SUBIR </span> <span class="badge">' + $.trim(infoResumen.nroNoSubidos) + '</span>';

                // si los bienes sin asignar son pocos los muestra
                if ( toNumero(infoResumen.nroBienes) > 0 && toNumero(infoResumen.nroNoSubidos) > 0 && toNumero(infoResumen.nroNoSubidos) < 5 ) {
                    html = html + '<br />';
                    html = html + '<span class="label label-warning">' + JSON.stringify(infoResumen.listaNoSubidos) + '</span>';
                }
    ...
    function eventGuardar() {
        ...
        $('#btn_guardar').on('click', function(e) {
            ...
            var cbo_estado_subida_guardar = $('#cbo_estado_subida_guardar').val();

            if ( cbo_estado_subida_guardar == '' || cbo_estado_subida_guardar == '-1' ) {

                showMensaje('', 'Por favor seleccione un Estado de Subida', function() {

                    $('#cbo_estado_subida_guardar').focus();
            ...
            showConfirmation('', '\u00BFSeguro que desea guardar los cambios?', function() {
                ...
                guardarAjax( selectedID, cbo_estado_subida_guardar);
    ...
    function guardarAjax(selectedID, estadoSubida) {

        $.ajax({
            url: 'estado-subida-inventario.htm?action=guardar',
            cache: false,
            data: {
    ...
    function eventBuscar() {
        ...
        $('#btn_buscar').on('click', function(e) {
            ...
            $.ajax({
                url: 'estado-subida-inventario.htm?action=validarBuscar',
                cache: false,
    ...
    function eventVer() {
        ...
        $('#btn_ver').on('click', function(e) {
            ...
            $(location).prop('href', 'estado-subida-inventario.htm?action=verInventarioBien&inventarioBienID=' + selectedID);
        });
    }
    ...
    function eventChangeLocal() {
        ...
        $("#cbo_local").change(function () {
            ...
            var formURL = 'estado-subida-inventario.htm?action=cargarAreas&formulario='+formulario;
    ...
    function eventChangeArea() {
        ...
        $("#cbo_area").change(function () {
            ...
            var formURL = 'estado-subida-inventario.htm?action=cargarOficinas&formulario='+formulario;
