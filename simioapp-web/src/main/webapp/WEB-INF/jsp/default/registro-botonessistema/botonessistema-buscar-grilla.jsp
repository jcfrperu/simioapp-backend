...
<table class="table table-striped table-bordered table-hover" id="botonessistema-buscar-grilla">

    <thead>
        <tr>
            <th title="Seleccionar Registro">SEL</th>
                        <th>BotonID</th>
            <th>NombreBot&oacute;n</th>
            <th>KeyBot&oacute;n</th>
    ...
    <tbody>
        ...
        <c:forEach items="${lista}" var="item" varStatus="status">
            ...
            <tr class="${claseFila}">
                <td class="text-center"><input type="radio" name="codigo" title="Seleccionar" value="${item.botonID}"></td>
                                <td>${item.botonID}</td>
                <td>${item.nombreBoton}</td>
                <td>${item.keyBoton}</td>
